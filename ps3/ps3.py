...
def get_word_score(word, n):
    ...
    first_component = sum([SCRABBLE_LETTER_VALUES[letter]
...
def deal_hand(n):
    ...
    # subtract 1 to account for a wildcard
...
def is_valid_word(word, hand, word_list):
    ...
    if word not in word_list:
        if '*' in word:
            ...
            if all([word not in word_list for word in possible_words]):
                return False
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    if calculate_handlen(hand) <= 0:
        print('\nRun out of letter. Total score:', total_score, 'points')
    else:
...
def substitute_hand(hand, letter):
    ...
    substitute = hand.copy()
    count = substitute.pop(letter, 0)

    if count == 0:
        return hand

    not_available_letters = list(hand.keys()) + [letter]

    available_letters = [
        letter for letter in string.ascii_lowercase if letter not in not_available_letters]

    random_available_letter = random.choice(available_letters)
    substitute[random_available_letter] = count
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    number_of_hands = int(input('Enter the number of hands: '))
